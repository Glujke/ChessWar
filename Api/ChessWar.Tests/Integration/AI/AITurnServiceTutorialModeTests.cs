...
/// <summary>
/// Тесты для проверки поддержки Tutorial режима в AITurnService
/// </summary>
public class AITurnServiceTutorialModeTests : IClassFixture<WebApplicationFactory<Program>>
{
    private readonly HttpClient _client;

    public AITurnServiceTutorialModeTests(WebApplicationFactory<Program> factory)
    {
    ...
    /// <summary>
    /// Тест: AITurnService должен поддерживать Tutorial режим, а не только AI режим
    /// </summary>
    [Fact]
    public async Task AITurnService_ShouldSupportTutorialMode_NotOnlyAIMode()
    {
        ...
        var mode = sessionDoc.RootElement.GetProperty("mode").GetString();
        
        Assert.Equal("Tutorial", mode);
